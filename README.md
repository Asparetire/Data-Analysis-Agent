...
- **结构化日志**：`LOG_FORMAT=json`（默认）输出带 `timestamp/level/logger/message/request_id` 的 JSON 行，方便 ELK/Loki 摄入；本地开发设 `LOG_FORMAT=text` 可读
- **request_id**：`RequestIdMiddleware` 为每个请求注入 UUID（读 `X-Request-ID` header 或生成），存入 `contextvars`，日志带同一 id，响应回写 `X-Request-ID` header
- **Prometheus metrics**：`/metrics` 暴露 `http_requests_total`、`http_request_duration_seconds`、`http_requests_in_progress`，生产用 Prometheus scrape（无鉴权，nginx 层限 IP）

### 6. 性能基线（Phase 5E，k6）

```bash
# 起 backend mock 模式 + 注册用户 + 上传 CSV，详见 perf/README.md
k6 run perf/k6-baseline.js
```

脚本对 `/health/ready`、`/auth/login`、`/datasources`、`/datasources/{id}/preview`、`/datasources/{id}/rows` 跑 20 VU 恒定 30s，内置 threshold（p95 health<200ms / login<1500ms / 其他<500ms）。LLM 端点不测（延迟由 provider 决定，基线无意义）。基线数填在 `docs/docs/performance-baseline.md`。

### 7. 文档站（MkDocs Material）
...
| GET  | `/api/v1/health/live` | liveness — 进程活着即 200，无依赖检查 |
| GET  | `/api/v1/health/ready` | readiness — 查 Redis + DB，任一挂返回 503，含 `version` + `uptime_seconds` |
| GET  | `/api/v1/health` | `/health/ready` 的向后兼容别名 |
| GET  | `/metrics` | Prometheus 文本格式指标（`/docs` Swagger 之外，无鉴权，生产在 nginx 层限制来源 IP） |
